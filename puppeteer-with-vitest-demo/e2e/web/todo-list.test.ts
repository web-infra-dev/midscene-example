...
describe('Test todo list', () => {
  ...
  it('ai todo', async () => {
    ...
    await ctx.agent.aiAct('click the check button on the left of the second task');
    await ctx.agent.aiAct(
      "click the 'completed' status button below the task list"
    );

    const list = await ctx.agent.aiQuery('string[], the complete task list');
    ...
    const name = await ctx.agent.aiString('What is the name of the first todo?');
    console.log('name', name);

    const todoCount = await ctx.agent.aiNumber(
      'How many todos are there in the list?'
    );
    console.log('todoCount', todoCount);

    const isAllCompleted = await ctx.agent.aiBoolean('Is all todos completed?');
    console.log('isAllCompleted', isAllCompleted);

    const location = await ctx.agent.aiLocate(
